...
function potential_value(physics::AbstractDict, x::Number)
...
function ern_r_of_rstar(rstar, M; tol=1e-14, maxiter=100)
    y = rstar / M
    yr = real(y)
    x = if abs(y) == 0 || abs(yr) < eps(Float64)
        1.0 + 0im
    elseif yr > 0
        yy = max(yr, 1.0)
        complex(max(yy - 2.0 * log(yy), 1e-8))
    else
        complex(max(-1.0 / yr, 1e-8))
    end
    for _ in 1:maxiter
        F = x - 1.0 / x + 2.0 * log(x) - y
        dF = (1.0 + 1.0 / x)^2
        xnew = x - F / dF
        if !isfinite(real(xnew)) || !isfinite(imag(xnew)) || abs(xnew) == 0
            xnew = x / 2.0
        end
