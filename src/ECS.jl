...
function ecs_kinetic_matrix(x::AbstractVector{<:Real}, ecs::ECSConfig)
    ...
    # Use the endpoint-defined spacing.  `collect(range(...; length=...))` is
    # mathematically uniform, but adjacent floating-point differences can vary by
    # a few ulps over long intervals.  A strict adjacent-difference check can
    # therefore reject grids generated by `range` itself.
    dx = (float(x[end]) - float(x[1])) / (nx - 1)
    tol = max(1e-12, 1000eps(Float64) * max(1.0, abs(dx), abs(x[1]), abs(x[end])))
    for j in 1:nx
        expected = float(x[1]) + (j - 1) * dx
        abs(float(x[j]) - expected) <= tol ||
            error("ecs_kinetic_matrix requires a uniform grid; maximum allowed deviation is $(tol).")
    end
